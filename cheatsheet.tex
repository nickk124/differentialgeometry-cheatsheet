...
% \theoremstyle{theorem}
% \begin{theorem}{\textit{Transformations of Coordinates and Differentials.}}
%     Let $f:M\rightarrow N$ be a differentiable/smooth map between smooth manifolds $M$ and $N$. Given some section $\omega$ of $T^*N$, we can define a section $\eta$ of $T^*M$ as
%     \begin{equation}
%         \eta(p) = \omega (f(p)) \circ f_{*p}
%     \end{equation}
...
\begin{definition}{\textit{Multilinear Function.}}
    ...
    If $V_i=V$ $\forall i$ for some $V$, this vector space is denoted $\mathcal{T}^m(V)$, the \textbf{space of all multilinear functions on $V$}.
\end{definition}
...
\begin{definition}{\textit{Tensor Product.}}
    Given some vector space $V$, for $T\in\mathcal{T}^k(V)$ and $S\in\mathcal{T}^\ell(V)$, the \textbf{tensor product} $T\otimes S\in\mathcal{T}^{k+\ell}(V)$ is defined by
    \begin{align}
